# esse aqui é online sempre deixa esse texto aqui para vc saber o outro é o local
...
PRINTER_URL = os.getenv("PRINTER_URL", "http://127.0.0.1:3334/print")
def normalizar_json(obj):
    ...
    return obj
def venda_tem_jogo_quiz(produtos):
    for p in produtos:
        nome = p.get("nome", "").lower()
        if "jogos&quiz" in nome or "jogos & quiz" in nome:
            return True
    return False

def buscar_ultimo_codigo_jogo():
    r = executar_select(
        """
        SELECT codigo
        FROM jogos
...
def gerar_pdf_comanda(venda_id, comercio, produtos, pagamento, total, codigo_jogo=None):
    if not venda_tem_jogo_quiz(produtos):
        codigo_jogo = None
    ...
    # PROTOCOLO
    # ===============================
    pdf.set_font("Helvetica", "B", FONT_DESTAQUE)
    ...
    if codigo_jogo:
        ...
        pdf.cell(W, 5, "CODIGO DO JOGO:", ln=True, align="C")
        pdf.set_font("Helvetica", "B", 11)
        pdf.cell(W, 6, codigo_jogo, ln=True, align="C")
        pdf.cell(W, 4, SEP, ln=True, align="C")
    ...
    return caminho
# ===============================
# TEXTO COMANDA (POS-58)
# ===============================
def gerar_texto_comanda(venda_id, comercio, produtos, pagamento, total, codigo_jogo=None):
    if not venda_tem_jogo_quiz(produtos):
        codigo_jogo = None
    linhas = []

    linhas.append(comercio["loja"].upper())
    linhas.append("-" * 32)
    linhas.append(f"VENDA {venda_id}")
    linhas.append(f"PAGAMENTO: {pagamento.upper()}")
    linhas.append("-" * 32)

    for p in produtos:
        linhas.append(p["nome"].upper())
        linhas.append(f"{p['quantidade']} x R$ {p['preco']:.2f}")
        linhas.append(f"SUBTOTAL: R$ {p['subtotal']:.2f}")
        linhas.append("")
    ...
    if codigo_jogo:
        linhas.append("-" * 32)
        linhas.append("CÓDIGO DO JOGO")
        linhas.append(codigo_jogo)

    agora = datetime.now(ZoneInfo("America/Sao_Paulo"))
    linhas.append(agora.strftime("%d/%m/%Y %H:%M"))
...
def finalizar_venda(dados: dict, usuario=Depends(verificar_token_cliente)):
    ...
    try:
        ...
        # STATUS INICIAL
        # ===============================
        status_inicial = "nfce_pendente" if maquininha else "aguardando_confirmacao"
        ...
        if venda_tem_jogo_quiz(dados["produtos"]):
            codigo_jogo = buscar_ultimo_codigo_jogo()
        ...
        # ===============================
        # IMPRIMIR COMANDA TÉRMICA (POS-58)
        # ===============================
  
        try:
            texto_comanda = gerar_texto_comanda(
                venda_id,
                comercio,
                dados["produtos"],
                dados["pagamento"],
                dados["valor"],
                codigo_jogo  # usa o mesmo já validado
            )
            ...
            if PRINTER_URL:
                try:
                    requests.post(
                        PRINTER_URL,
                        json={"text": texto_comanda},
                        timeout=3
                    )
                    log(f"Comanda térmica enviada para impressão da venda {venda_id}")
                except Exception as e:
                    log(f"Falha ao imprimir comanda térmica: {e}")
            else:
                log("Impressora não configurada")
        ...
        log(str(e))
        raise HTTPException(status_code=500, detail="Erro interno ao finalizar venda")
...
@router.post("/vendas/{venda_id}/confirmar")
def confirmar_venda(venda_id: int, usuario=Depends(verificar_token_cliente)):
    ...
    venda = executar_select(
        ...
        SELECT v.id, c.node
        FROM vendas_ib v
        JOIN clientes cl ON cl.id = v.realizada
        JOIN comercios_cadastradas c ON c.id = v.empresa
        WHERE v.id = %s
        """,
        (venda_id,)
    )
    ...
    executar_comando(
        "UPDATE vendas_ib SET status = %s WHERE id = %s",
        ("nfce_pendente", venda_id)
    )

    return {
        "ok": True,
        "venda_id": venda_id,
        "status": "nfce_pendente",
        "imprimir": node == 1
    }

@router.get("/vendas/para-impressao")
def vendas_para_impressao():

    vendas = executar_select(
        """
        SELECT id, comanda
        FROM vendas_ib
        WHERE status = 'nfce_pendente'
          AND comanda IS NOT NULL
        ORDER BY id ASC
        """
    )
...
@router.delete("/vendas/{venda_id}/cancelar")
def cancelar_venda(venda_id: int, usuario=Depends(verificar_token_cliente)):

    venda = executar_select(
        "SELECT id FROM vendas_ib WHERE id = %s",
        (venda_id,)
    )
    ...
    executar_comando(
        "DELETE FROM vendas_ib WHERE id = %s",
        (venda_id,)
    )
...
@router.post("/vendas/{venda_id}/liberar-impressao")
def liberar_impressao(venda_id: int, usuario=Depends(verificar_token_cliente)):

    if "id" not in usuario:
        raise HTTPException(status_code=401, detail="Usuário inválido")

    venda = executar_select(
        "SELECT status, comanda FROM vendas_ib WHERE id = %s",
        (venda_id,)
    )
    ...
    if venda["status"] != "aguardando_confirmacao":
        raise HTTPException(
            status_code=409,
            detail="Venda não está aguardando confirmação"
        )

    if not venda["comanda"]:
        raise HTTPException(
            status_code=409,
